...
def mod_mod_2upscl(inp:Layer, image_shape:tuple, image_channels:int, kernel_initializer:Initializer=RandomNormal(stddev=0.02)):
	...
	# (256 * st_s^2,) -> (st_s, st_s, 256)
	m = Dense(256 * st_s[0] * st_s[1], kernel_initializer=kernel_initializer, activation="relu")(inp)
	m = Reshape((st_s[0], st_s[1], 256))(m)
	m = BatchNormalization(momentum=0.8)(m)

	# (st_s, st_s, 256) -> (st_s, st_s, 256)
	m = Conv2D(256, (3, 3), padding="same", kernel_initializer=kernel_initializer, activation="relu")(m)
	m = BatchNormalization(momentum=0.8)(m)

	# (st_s, st_s, 256) -> (2*st_s, 2*st_s, 128)
	m = UpSampling2D()(m)
	m = Conv2D(128, (3, 3), padding="same", kernel_initializer=kernel_initializer, activation="relu")(m)
	m = BatchNormalization(momentum=0.8)(m)

	# (2*st_s, 2*st_s, 128) -> (4*st_s, 4*st_s, 64)
	m = UpSampling2D()(m)
	m = Conv2D(64, (3, 3), padding="same", activation="relu", kernel_initializer=kernel_initializer)(m)
	m = BatchNormalization(momentum=0.8)(m)

	# (4*st_s, 4*st_s, 64) -> (4*st_s, 4*st_s, num_ch)
	m = Conv2D(image_channels, (3, 3), padding="same", kernel_initializer=kernel_initializer, activation="tanh")(m)
	return m
...
def mod_base_3upscl(inp:Layer, image_shape:tuple, image_channels:int, kernel_initializer:Initializer=RandomNormal(stddev=0.02)):
	...
	m = Dense(128 * st_s[0] * st_s[1], kernel_initializer=kernel_initializer, activation="relu")(inp)
