...
from modules.custom_layers import deconvolutional_block, identity_block_skip
...
def mod_ext_3upscl(inp:Layer, image_shape:tuple, image_channels:int, kernel_initializer:Initializer=RandomNormal(stddev=0.02)):
	...
	m = deconvolutional_block(m, k=3, filters=[1024, 512, 512])

	m = identity_block_skip(m, k=3, filters=[128, 256, 512])
	m = LeakyReLU(0.2)(m)

	# (2*st_s, 2*st_s, 512) -> (4*st_s, 4*st_s, 256)
	m = deconvolutional_block(m, k=3, filters=[512, 256, 256])

	m = identity_block_skip(m, k=3, filters=[64, 128, 256])
	m = LeakyReLU(0.2)(m)

	# (4*st_s, 4*st_s, 256) -> (8*st_s, 8*st_s, 128)
	m = deconvolutional_block(m, k=3, filters=[256, 128, 128])

	# (8*st_s, 8*st_s, 128) -> (8*st_s, 8*st_s, 128)
	m = identity_block_skip(m, k=3, filters=[32, 64, 128])
	m = LeakyReLU(0.2)(m)
