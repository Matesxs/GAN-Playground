...
from modules.custom_layers import deconvolutional_block, identity_block
...
def mod_ext_3upscl(inp:Layer, image_shape:tuple, image_channels:int, kernel_initializer:Initializer=RandomNormal(stddev=0.02)):
	...
	m = deconvolutional_block(m, k=3, filters=[512, 512, 512])

	# (2*st_s, 2*st_s, 512) -> (4*st_s, 4*st_s, 256)
	m = deconvolutional_block(m, k=3, filters=[256, 256, 256])

	# (4*st_s, 4*st_s, 256) -> (8*st_s, 8*st_s, 128)
	m = deconvolutional_block(m, k=3, filters=[128, 128, 128])

	# (8*st_s, 8*st_s, 128) -> (8*st_s, 8*st_s, 128)
	m = identity_block(m, k=3, filters=[32, 64, 128])
	m = LeakyReLU(0.2)(m)

	# (8*st_s, 8*st_s, 128) -> (8*st_s, 8*st_s, 128)
	m = identity_block(m, k=3, filters=[16, 32, 128])
	m = LeakyReLU(0.2)(m)

	# (8*st_s, 8*st_s, 16) -> (8*st_s, 8*st_s, image_channels)
	m = Conv2D(image_channels, kernel_size=(3, 3), padding="same", activation="tanh", kernel_initializer=kernel_initializer)(m)
	return m